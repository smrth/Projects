...
LSystem::LSystem(const string &filename) :
  ...
  sentence( ) 
  {
    ...
    fillSprite();
    //cout << "System builded" << endl;

}

LSystem::~LSystem() {
  //SDL_FreeSurface(sprite1Surface);
...
void LSystem::fillSprite() { 
  SDL_LockSurface(sprite1Surface);
  ...
  SDL_FillRect(sprite1Surface, NULL, MAGENTA);
  state.x = startX;
  state.y = startY;
  for (unsigned int i = 0; i < sentence.size(); ++i) {
    switch ( sentence[i] ) {
      ...
      case 'F' :  case 'A' : case 'B' : {
        ...
        Draw_AALine(sprite1Surface, state.x,state.y, x1,y1, strokeWidth,
                    strokeColor);
        state.x = state.x + (x1-state.x);
        state.y = state.y + (y1-state.y);
        break;
  ...
  SDL_UnlockSurface(sprite1Surface);
}

void LSystem::draw() const { 
  SDL_Rect src = { 0, 0, width, height };    
  SDL_Rect dest = {0, 0, width, height };
  SDL_BlitSurface(sprite1Surface, &src, screen, &dest);
}
